...
def _step(
    ...
    # TODO(wendyshang): also record the norm of updates
    ...
    # TODO(wendyshang): make update interval a hyperparameter
...
class RayTrainerShard:
    ...
    def initialize(self):
        try:
            ...
            logging.info("Initializing RayTrainerShard. Environment: %s", os.environ)
            ...
            # TODO(wendyshang): make loader more efficient
            ...
            print("Done init learner")
            logging.info("Done init learner")
    ...
    def train(self):
        ...
        for train_step in range(
            ...
            all_steps = self.ckpt_manager.all_steps(read=True)
            latest_step = max(all_steps) if all_steps else None
            logging.info(f"trainer_saved_latest_step={latest_step}")
    ...
    def __repr__(self):
        return f"[RayTrainerShard-w{self.shard_id}]"


@ray.remote
class RayTpuTrainer:
    """Global manager for all trainer shards.
    Attributes:
        reverb_server_address: The address of the Reverb server.
        save_dir: Directory where to save logs.
        ckpt_dir: Directory where to save checkpoints.
    ...
    def __init__(
        ...
        logging.info("Initializing RayTpuTrainer")
        tpu_id = "trainer"
        num_trainers = int(ray.available_resources()[tpu_id])
        logging.info("Number of detected trainers: %d", num_trainers)
        print("Number of detected trainers: ", num_trainers)
        self._trainers = [
            RayTrainerShard.options(resources={"TPU": 4, tpu_id: 1}).remote(
                shard_id=i,
    ...
    def initialize(self):
        ...
        return ray.get([t.initialize.remote() for t in self._trainers])

    def train(self):
        # Do we want to return anything?
        logging.info("Starting to train")
        return ray.get([t.train.remote() for t in self._trainers])

    def __repr__(self):
        return f"[RayTpuTrainer]({socket.gethostname()})"
