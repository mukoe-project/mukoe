...
History = Mapping[str, Mapping[str, Event]]  # owner_name -> event_category -> event_id -> Event
...
class EventLogger:
    ...
    def event_start(self, owner_name: str, event_category: str, timestamp: float, event_id: Optional[str] = None):
    ...
    def event_stop(self, owner_name: str, event_category: str, timestamp: float, event_id: Optional[str] = None):
    ...
    def get_latest_event_id(self, owner_name: str, event_category: str) -> Optional[str]:
        """
    ...
    def summary(self):
        for actor, actor_history in self.history.items():
            ...
            for event_category, event_history in actor_history.items():
                ...
                event_durations = [event.stop - event.start for event_id, event in event_history.items() if event.stop is not None]
...
def initialize():
    ...
    global_logger = EventLogger.options(
        lifetime="detached",
        name="event_logger",
        namespace="metrics").remote()
